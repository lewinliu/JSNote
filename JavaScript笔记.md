...
- shift() : 从数组中移除第一个元素并返回它；
  ...
  @param
  @return
   	从数组中移除第一个元素并返回它。如果数组为空，则返回undefined，并且不修改数组。
  ```
- pop() : 从数组中移除最后一个元素；
...
- slice() ：返回数组某个部分的副本。
  ...
  /* 
  思考:
      1.如果 start || end 为负数会咋地?
        如果 start > end 了会咋地?
        如果是小数会咋地?
        如果是非有效数字会咋地?
        如果 end 或者 start 的值比最大索引都大会咋地?
